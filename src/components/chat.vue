    ...
    receive(msg) {
      ...
      if (message instanceof Promise) {
        message.then(
          message =>
            this.appendNew(
              Object.assign({ time: new Date(), direction: 'received' }, message)
            )
        ).catch(e => console.error('接受消息出错', e))
      } else {
        this.appendNew(Object.assign({ time: new Date(), direction: 'received' }, message))
      }
    },
